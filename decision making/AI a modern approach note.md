...
2. **Unsupervised clustering**: the first example of EM, **Gaussian Mixture Models**.
![unsupervised clustering](<unsupervised clustering.png>)
    - Problem statement. The data are generated from a mixture distribution, and in this case, there are three Gaussian mixture component. The distribution is given by $P(X)=\sum_{i=1}^3P(C=i)P(X|C=i)$.
    - *E-step*: for each data point ($j=1,\cdots, N$) and each Gaussian component ($i=1,2,3$): 
        - compute the probabilities that $x_j$ is generated by component $i$: $P(C=i|x_j)$. By Bayes' rule, $P(C=i|x_j)=\alpha P(x_j|C=i)P(C=i)$, where  $ P(x_j|C=i)$ is the probability at $x_j$ of the component $i$ and $P(C=i)$ is the weight parameter for component $i$.
        - define $n_i = \sum_{j=1}^N P(C=i|x_j)$, the effective number of data points assigned to component $i$.
    - *M-step*: update the parameters of each Gaussian component and weighted by their effective number of data points.
3. **Learning Bayes net parameter vaules for hidden variables**:
    - *Initialization*: Define the structure of the Bayesian network (if not already defined).
    Initialize the parameters of the network, i.e., the probabilities in the conditional probability tables (CPTs) associated with each node. These initial values can be set based on prior knowledge or randomly.
    - *E-Step*: Calculate the expected value of the complete-data log-likelihood function. This involves computing the posterior probabilities of the hidden variables given the observed data and the current estimate of the model parameters. Essentially, you're using the current model parameters to infer the distribution of the hidden variables.
    - *M-Step*: Update the model parameters to maximize the expected log-likelihood computed in the E-step. This typically means updating the CPTs to reflect the probabilities suggested by the combined observed and inferred data distributions. Parameters are updated such that they are most likely given the observed data and the current estimate of the hidden data.
    - *Iteration*: Repeat the E-step and M-step until the changes in the parameters are minimal or the improvement in the log-likelihood is beneath a certain threshold, indicating convergence.
4. **Learning hidden Markov models**: required further study of Section 14.3.
5. **The general form of the EM algorithm**: Let $Z$ denote all hidden variables, and $\theta$ be all parameters of the probability model. Then, EM algorithm is $$\theta^{i+1}=\argmax_\theta\sum_zP(Z=z|x,\theta^i)L(x,Z=z|\theta)$$
    The key factor is to find the hidden distribution of latent variables.

## Deep Learning
### Simple Feedforward Networks
1. Networks as complex functions